...
_.randomProfile = function (gender) {
    ...
    const index =
        gender === 'male'
            ? _.randomFromArray([1, 2, 3, 8, 9])
            : _.randomFromArray([4, 5, 6, 7, 10]);
...
_.deepClone = function (data) {
    ...
    if (originalData.constructor === Object)
        return this.cloneObject(originalData);
    if (originalData.constructor === Array)
        return this.cloneArray(originalData);
    return originalData;
};
...
) => `${iTableId}:scheduler:${sTask}:${iUserId}:${host}`;
